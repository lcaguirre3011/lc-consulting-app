...
export function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const convertLeadToClient = useCallback((leadId: string) => {
    setData((current) => {
      ...
      const leadIndustry = lead.intake?.industry ?? lead.industry;
      const leadProblem = lead.intake?.mainProblem ?? lead.desiredOutcome ?? lead.nextStep;
      const leadGoal = lead.intake?.expectedResult ?? lead.desiredOutcome ?? "";
      const newClient: Client = {
        id: clientId,
        name: lead.company,
        industry: leadIndustry ?? (lead.intake?.howFoundUs === "Evento" ? "Servicios" : "Por clasificar"),
        ...
        accountOwner: lead.assignedTo ?? "Laura Cardenas",
        mainPain: leadProblem,
        ...
        contacts: [
          {
            ...
            role: lead.contactRole ?? "Contacto principal",
            email: lead.email,
            phone: lead.phone,
          },
          ...current.contacts,
        ...
        intakeForms: [
          {
            ...
            urgency: lead.urgency ?? (lead.intake?.internalFlag === "verde" ? "media" : "alta"),
            budget: lead.budget ?? lead.intake?.budgetAvailable ?? "Pendiente",
        ...
        diagnoses: [
          {
            ...
            intakeId,
            company: lead.company,
            summary: `Primer expediente clinico-empresarial creado desde el filtro inicial. El paciente presenta: ${leadProblem}`,
            detectedProblems: [leadProblem || "Problema principal pendiente de descubrir"],
            opportunities: [leadGoal || "Definir resultado esperado en sesion"],
